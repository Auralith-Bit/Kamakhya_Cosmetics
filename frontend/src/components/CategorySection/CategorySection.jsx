...
/* 18 items like the video – last page ends Hair Care / Sun Protection / Face Care */
const CATEGORIES = [
...
const CategorySection = () => {
  ...
  /* count pages = ceil(content / viewport) */
  useEffect(() => {
  ...
  const scrollToPage = (i) => {
    ...
    el.scrollTo({ left: Math.min(i * el.clientWidth, max), behavior: 'smooth' });
    setActiveIndex(i);
  };
      ...
      <div className="px-[125px] max-lg:px-8 max-sm:px-5">
        ...
        >
          {CATEGORIES.map((cat, i) => (
            <div key={i} style={{ flexShrink: 0, width: 'calc((100% - 112px) / 5)', minWidth: '210px' }}>
              {/* white card = image only */}
              <div
                className="cat-card"
                style={{
              ...
              {/* label BELOW the card, like the video */}
              <p style={{
